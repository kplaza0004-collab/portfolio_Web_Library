...
public interface RentalRepository extends JpaRepository<Rental, Long> {
    ...
    /**
     * 月別実績集計用の検索クエリ (AdminController.calculateMonthlyStats 用)
     */
    ...
    /**
     * 会員別レンタル実績ベスト10を取得
     * LPAD(r.user_id, 8, '0') を使用して 8桁のゼロ埋め表示 に整形します。
     */
    @Query(value = "SELECT LPAD(r.user_id, 8, '0') AS userName, COUNT(r.id) AS rentalCount " +
                   "FROM rentals r " +
                   "GROUP BY r.user_id " +
